class Account:
    ...
    def time_to_retire(self, amount):
        ...
        while left < right:
            mid = (left + right) // 2
            if balance * (1 + interest) ** mid < amount:
                left = mid + 1
            else:
                right = mid
        return left
...
class FreeChecking(Account):
    ...
    "*** YOUR CODE HERE ***"
    def withdraw(self, amount):
        amount = amount + (self.withdraw_fee * (self.free_withdrawals == 0))
...
def without(s, i):
    ...
    "*** YOUR CODE HERE ***"
    dummy = Link(0)
    idx, cur, tmp = 0, dummy, s
    while tmp:
        if idx != i:
            cur.rest = Link(tmp.first)
            cur = cur.rest
        tmp = tmp.rest
        idx += 1
    return dummy.rest
...
def duplicate_link(s, val):
    ...
    while cur:
        if cur.first == val:
            cur.rest = Link(cur.first, cur.rest)
            cur = cur.rest
        cur = cur.rest
