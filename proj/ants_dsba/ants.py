...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        cnt = 0
        while not place.is_hive and cnt <= self.upper_bound:
            if place.bees and cnt >= self.lower_bound:
...
# The WallAnt class
class WallAnt(Ant):
    """ WallAnt adds some protection to our glorious home. """
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        target.slow = self.slow_turn
        def modified(gamestate):
            if gamestate.time % 2 and target.slow:
                pass
            else:
