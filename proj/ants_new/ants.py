...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if self and not self.is_doubled:
            self.damage *= 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        distance = 0
        while distance < self.lower_bound and not place.is_hive:
            place = place.entrance
            distance += 1
        while not (place.bees or place.is_hive or distance > self.upper_bound):
            place = place.entrance
            distance += 1
        return random_bee(place.bees) if not place.is_hive and self.lower_bound <= distance <= self.upper_bound else None
        # END Problem 3 and 4
    ...
    def throw_at(self, target):
        ...
        def action(gamestate):
            ...
            if target.slow_time > 0:
                ...
                Bee.action(target, gamestate)
        if target:
            target.slow_time = self.slow_duration
