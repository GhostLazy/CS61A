...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def select(s):
        s_list = [lower(x) for x in split(remove_punctuation(s))]
        for x in subject:
            if x in s_list:
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    return word_list[idx] if diff[idx] <= limit else typed_word
    # END PROBLEM 5
...
def time_per_word(words, timestamps_per_player):
    ...
    for k in range(len(tpp)):
        times_player_k = []
        for i in range(0, len(tpp[k])-1):
            times_player_k.append(tpp[k][i+1] - tpp[k][i])
        times.append(times_player_k)
    # END PROBLEM 9
